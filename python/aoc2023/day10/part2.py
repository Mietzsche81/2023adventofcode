...
encode = {c: i for i, c in enumerate(".|-LJ7FS")}
decode = {i: c for c, i in encode.items()}

grid = np.array(
    [
        [encode[char] for char in stripped]
        for line in lines
        if (stripped := line.strip())
    ]
)
size = grid.shape
start = np.argwhere(grid == encode["S"])[0]
...
def neighbors(cell):
    ...
    return [
        ...
        if (grid[*absolute] in (encode[char] for char in valid[relative]))
    ]


def advance(previous, current):
    delta = current - previous
    match decode[grid[*current]]:
        case "|":
            if delta[0] > 0:
                return current + [1, 0]
            else:
...
loop: list[np.ndarray] = [start, neighbors(start)[0]]
while not (loop[-1] == start).all():
    loop.append(advance(*loop[-2:]))

# TODO algorithm, cheating here
start_type = "|"
grid[*start] = encode[start_type]

loop = [tuple(i) for i in loop]
contained = 0
for row in range(size[0]):
    internal = False
    for col in range(size[1]):
        if (row, col) in loop:
            if decode[grid[row, col]] in "|JL":
